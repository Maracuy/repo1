...
if ($_SESSION['user']['nivel'] == "Super Admin"){
    $sql_query = $con->prepare('SELECT * FROM empleados');
    $sql_query->execute();
    $resultado = $sql_query->fetchALL();
}else{
    $sql_query = $con->prepare("SELECT * FROM empleados WHERE nivel != 'Super Admin'");
    $sql_query->execute();
    $resultado = $sql_query->fetchALL();
}
;
...
<body>
    ...
    <?php include 'controlador/muestraempleados.php'?>
